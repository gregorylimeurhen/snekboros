...
If you have CUDA, check its version with:

```bash
nvidia-smi
```

One installation option is to use `uv` by following the installation instructions [here](https://docs.astral.sh/uv/getting-started/installation/).
...
The `pyproject.toml` file is configured to target CUDA 12.8:
...
If you are targeting a different CUDA version, or CPU only, refer to the [uv documentation](https://docs.astral.sh/uv/guides/integration/pytorch/#installing-pytorch) and modify the `pyproject.toml` file accordingly.

Another installation option is to use `pip` and `venv`.
...
Alternatively, we can also train a custom vision encoder from scratch in a self-supervised manner, that is JEPA. This allows us to obtain more task-specific hidden representations. Given that the Snake game environment is relatively simple and low-dimension, 16 x 16 grid, the vision encoder can be trained directly on this native resolution, as opposed to upsampling for DINO's 224 x 224 input, resulting in a much smaller observation model. For such low resolution images, even a 2 x 2 convolutional kernel might be too large, as it would already cover a significant portion of the image. We could use a simple ViT architecture with a patch size of 1, that is each pixel is treated as a patch, allowing each pixel to attend to every other pixel in the image.

### Dynamics Model

The transition model captures the dynamics of the environment, allowing us to simulate future states based on current observations and actions. The transition model takes in a history of past latent states $z_{t-H:t}$ and actions $a_{t-H:t}$, where $H$ is a hyperparameter that determines the history or context length, and predicts the latent state at the next time step $z_{t+1}$.

We adopt a ViT architecture without the tokenisation step, since we already have the patch embeddings, observation latent states, from the observation model. We flatten each latent state $z_t \in \mathbb{R}^{N \times E}$ into a sequence of patch embeddings $z_t^i \in \mathbb{R}^E$ for $i = 1, \ldots, N$, essentially treating each patch embedding as a token. As a result, the transition model receives a sequence of length $N \times H$ as input.

To capture temporal dependencies between the latent states, we use a causal attention mask. Specifically, each patch embedding $z_t^i$ for the latent state $z_t$ attends to $\{ z^i_{t-H:t-1} \}_{i=1}^{N}$ but **not** $\{ z_t^i \}_{i=1}^{<k}$, where $k$ is the index of the current patch embedding. The idea is that treating patch vectors of one observation as a whole better captures temporal dynamics, as opposed to allowing each patch embedding to attend to all other patch embeddings in the same time step.

As mentioned previously, the transition model also takes in a history of past actions $a_{t-H:t}$, which are crucial for predicting the next state. There are two approaches for passing in action information to the transition model. One way is to encode actions, for example using an MLP, to a higher dimensional representation, that is $\phi(a_t) \in \mathbb{R}^{K}$, and then simply concatenate the action vector to each patch vector of the corresponding latent state. The concatenations $[z_t^i; \phi(a_t)] \in \mathbb{R}^{E+K}$, for all patches and time steps, are passed into the ViT as tokens. This way, actions are treated as additional features for each patch embedding, allowing the model to learn how actions influence the state transitions. The alternate approach is to treat the encoded actions $\phi(a_t) \in \mathbb{R}^{K}$ as separate additional tokens. This requires the encoded actions to be in the same dimensional space as the patch embeddings, that is $K = E$, so that they can be concatenated along the sequence dimension and passed into the ViT. Any available propioceptive information can also be encoded and incorporated into the input in a similar manner.
