...
Consider the standard transformer \citep{vaswani2017attention}:
\begin{itemize}
	...
	$$
	\item The real world often has nothing to do with language \citep{einstein1931world}; humans use discrete, low-bandwidth, and clear symbols for communication, but the real world is often continuous, high-bandwidth, and noisy \citep{lecun2025jepa}.
\end{itemize}
\end{frame}
...
\begin{frame}
What we shall discuss has been discussed in many works, including talks by LeCun.
\begin{figure}
	\centering
	\includegraphics[width=0.99\textwidth]{lecun-talk.png}
\end{figure}
We identify 2 published works which, as far as we can tell, sit closest to our work in task and method:
\begin{itemize}
	\item \textbf{Multiplayer Snake ... using Self-Play} \citep{francis2019multiplayer} has our task but uses PPO.
	\item \textbf{TD-MPC} \citep{hansen2022tdmpc} has our method but uses continuous-control DMControl tasks.
\end{itemize}
\end{frame}
...
\begin{itemize}
	\item The encoder predicts a hidden representation from an image.
...
Let $\mathrm{H}\in\N_{\geq 1}$ be a hidden space dimensionality,
...
\begin{align*}
	...
	\mathscr{F}
	&:
	\mathcal{H}\times\mathcal{A}\times\mathcal{Z}
	\to
	\mathcal{H}
	\tag{world model}
	\\
...
\begin{frame}{Project Timeline Plan}
	\begin{itemize}
		\item Week 9: Data-related implementation.
		\item Week 10: Model-related implementation.
		\item Week 11: Model training and evaluation.
		\item Week 12: Report and slide deck preparation.
	\end{itemize}
\end{frame}

\begin{frame}{Project Task Distribution}
	\begin{itemize}
		\item Celest: Training and testing of model(s).
		\item Ryan: Implementation of model(s).
		\item Yihe: Implementation of data functions.
		\item Gregory: Writing for report and slide deck, and so on.
	\end{itemize}
\end{frame}
